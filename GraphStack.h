...
class GraphStack {
    public:
        ...
        int contentoffset;
        ContentNode* contentlist;
        int GLRParse(SimpleLR* slr, int* input,char** eleminput, char**geleminput, int n, std::vector<ParseInfo>&parseinfos);
        std::vector<PathNode*> GetPathByLength(GraphStackNode *node, int nlen);
        std::string GetContentString(GraphStackNode *node, PathNode* path);
