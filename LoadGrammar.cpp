...
int Grammar::LoadGrammarRules(const char* path)
{
    ...
    while (fgets(line,LINE_LEN_1,fin))
    {
        ...
        {
            m_rules.insert(std::make_pair(line,id));
            m_id2rules.insert(std::make_pair(id,line));
            id++;
        }
    }
...
int Grammar::LoadNonterminals(const char* path)
{
    ...
    while (fgets(line,LINE_LEN_1,fin)) 
    {
        ...
        {
            m_nonterminals.insert(std::make_pair(line,id));
            id++;
        }
    }
    if (fin != NULL)
...
int Grammar::LoadTerminals(const char * path)
{
    ...
    while (fgets(line,LINE_LEN_1,fin)) 
    {
        ...
        {
            m_terminals.insert(std::make_pair(line,id));
            id++;
        }
    }
...
{
    for (std::unordered_map<std::string,int>::iterator it = m_nonterminals.begin();it != m_nonterminals.end(); it++ )
    {
        m_symbols.insert(it->first);
    }
    for (std::unordered_map<std::string,int>::iterator it = m_terminals.begin();it != m_terminals.end(); it++ )
    {
        m_symbols.insert(it->first);
    }
    return 0;
...
*elems: B,b
******************************************************************************************/
int Grammar::GetFirstElemInRuleBody(std::string head,std::unordered_set<std::string> &elems) 
{
    for (std::unordered_map<std::string,int>::iterator it = m_rules.begin(); it != m_rules.end();it++)
    {
...
bool Grammar::isAllTerminals(std::unordered_set<std::string> &itsset, std::vector<std::string>&nontermvec)
{
    bool flag = true;
    for (std::unordered_set<std::string>::iterator it = itsset.begin(); it !=itsset.end(); it++)
    {
...
int Grammar::GetSingleFirst(std::string head,std::string &firststr) 
{
    ...
    {
        int i = 0;
        for (std::unordered_set<std::string>::iterator it = elems.begin(); it != elems.end();it++) 
        {
            firststr += *it;
    ...
    else 
    {
        ...
        std::unordered_set<std::string> headsInStack;
        ...
        while (!mystack.empty()) 
        {
            ...
            if (index == -1) 
            {
                ...
                if (stNodeOld != NULL) 
                {
                    ...
                    std::vector<std::string> oldtempvec;
            ...
            {
                    std::string deshead = tempvec[index];
                    elems.clear();
                    nontermvec.clear();
...
int Grammar::GetFirstSet() 
{
    for (std::unordered_map<std::string,int>::iterator it = m_nonterminals.begin(); it != m_nonterminals.end(); it++) 
    {
        ...
        for (std::vector<std::string>::iterator it2 = tempvec.begin(); it2 != tempvec.end(); it2++) 
        {
            tempset.insert(*it2);
        }
        firststr = "";
        int count = 0;
        for (std::unordered_set<std::string>::iterator it2 = tempset.begin(); it2 != tempset.end(); it2++) 
        {
            firststr += *it2;
        ...
        m_firstset.insert(std::make_pair(it->first, firststr));
    }
    return 0;
}
int Grammar::GetHeadAndNeighborSet(std::string bodyelem, std::unordered_set<std::string> &headelems,std::unordered_set<std::string> &neighborelems) 
{
    for (std::unordered_map<std::string,int>::iterator it = m_rules.begin(); it != m_rules.end();it++) 
    {
        std::vector<std::string> rules_vec;
 ...
 ***************************************************/
int Grammar::GetSingleFollow(std::string elem, std::string &followstr) {
    std::unordered_set<std::string> headelems;
    std::unordered_set<std::string> neighborelems;
    GetHeadAndNeighborSet(elem,headelems,neighborelems);
    followstr = "";
    std::string temp = "";
    for (std::unordered_set<std::string>::iterator it = neighborelems.begin(); it != neighborelems.end(); it++)
    {
        if (m_nonterminals.count(*it)) 
        {
            temp+=m_firstset[*it];
    ...
    if (headelems.size() > 0) 
    {
        std::unordered_set<std::string> keyInStack;
        StackNode *stNode = NULL;
        StackNode *stNodeOld = NULL;
        stNode = new StackNode;
        stNode->key = elem;
        ...
        int count = 0;
        for (std::unordered_set<std::string>:: iterator it = headelems.begin(); it != headelems.end();it++) 
        {
        ...
        while (!mystack.empty()) 
        {
            ...
            if (-1==index) 
            {
                ...
                {
                    strncat(stNodeOld->symbollist,stNode->symbollist,strlen(stNode->symbollist));
                    std::vector<std::string> oldelemvec;
...
int Grammar::GetFollowSet()
{
    ...
    for (std::unordered_map<std::string,int>::iterator it = m_nonterminals.begin(); it != m_nonterminals.end(); it++) 
    {
        ...
        m_followset.insert(std::make_pair(it->first, followstr));
    }
    return 0;
}
