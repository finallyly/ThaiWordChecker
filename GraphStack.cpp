...
void GraphStack::clear( ) 
{
    ...
    p = head;
    std::queue<GraphStackNode*> myque;
    std::stack<GraphStackNode*> mystack;
...
// a node given ,find
std::vector<PathNode*> GraphStack::GetPathByLength(GraphStackNode *node, int nlen) 
{
    std::vector<PathNode*> node_vec;
    std::stack<GraphStackNode*> mystack;
...
std::string GraphStack::GetContentString(GraphStackNode*node,PathNode*path) {
...
int GraphStack::GLRParse(SimpleLR* slr, int* input ,char** eleminput, char**geleminput,int n,std::vector<ParseInfo>& parseInfos) {
    int ip = 0;
    while (ip <= n) {
//        fprintf(stderr,"eleminput=%s,geleminput=%s\n",eleminput[ip],geleminput[ip]);
        ...
        while (pawork) {
            ...
            if (0!=strcmp(actionstr,"")) {
                ...
                for (std::vector<std::string>::iterator it = actionlist.begin(); it != actionlist.end(); it++) {
                    if ((*it).at(0) == 'r') {
                        //fprintf(stdout," for reduce state_id=%d,gid=%d,gelem=%s, actionstr=%s, str[%d]=%d\n",state_id,gid,gelem.c_str(),actionstr,ip,input[ip]);
                      std::string ruleidstr = (*it).substr(1);
                      int rule_id = atoi(ruleidstr.c_str());
                      std::string ruletext = (slr->m_id2rules)[rule_id];
                      ...
                      for (std::vector<PathNode* >::iterator subit = node_vec.begin(); subit != node_vec.end(); subit++) {
                        ...
                        while (edge) {
                            ...
                            if (!createnewContent) {
                                ...
                                std::string temp =GetContentString(pnode,(*subit));
                     ...
                     // end of traverse path
                      ...
                      if (pawork->actioncount==int(actionlist.size())) {
                        for (std::vector<PathNode*>::iterator it = node_vec.begin();it!=node_vec.end();
                        it++) {
                      ...
                      // free pathnode;
                      for (std::vector<PathNode*>::iterator it = node_vec.begin();it!=node_vec.end();it++) {
                          ClearPath((*it));
                      }
                    //end of reduce;
                    }
                ...
                for (std::vector<std::string>::iterator it = actionlist.begin(); it != actionlist.end(); it++) {
                    if ((*it).at(0) == 's') {
                     //fprintf(stdout,"for shift state_id=%d,gid=%d,gelem=%s,actionstr=%s,str[%d]=%d\n",state_id,gid,gelem.c_str(),actionstr,ip,input[ip]);
                      std::string desstateidstr = (*it).substr(1);
        ...
        if (ip == n) {
            ...
            //fprintf(stdout, "****************************************************************************************************\n");
            if (0 == flagerr) {
               ParseTree parseTree("Stcc");
               parseTree.ConstructParseTree(contentlist, contentoffset - 1);
               ParseInfo info;
               info.treeStr = parseTree.RetrieveParseTree();
               std::vector<std::pair<std::string,std::string> > allresults=parseTree.RetrieveSyllablesFromTree("tcc");
               for (int i = 0; i < allresults.size();i++)
               {
                 ...
                 info.syllables+= allresults[i].first;
                 info.grammarsymbols += allresults[i].second;
               }
               parseInfos.push_back(info);
                /*
