...
SimpleLR::SimpleLR(std::string startsymbol): Grammar(startsymbol) {
...
int SimpleLR::Closure(std::unordered_set<std::string>&itemSet)
{
    std::queue<std::string> tempque;
    std::set<std::string> SymbolVisited;
    //copy the rules in the initial itemSet to the tempque
    for (std::unordered_set<std::string>::iterator it=itemSet.begin();
        it!=itemSet.end();it++ ) 
    {
        tempque.push(*it);
    }
    ...
    while(!tempque.empty()) 
    {
        ...
        std::string expelem;//the Grammer Symbol to expand
        ...
        {
            for (std::unordered_map<std::string,int>::iterator it = m_rules.begin();
             it != m_rules.end();it++)
             {
                std::vector<std::string> rules_vec;
                ...
                {
                    std::string newrule = expelem+":dott";
 ...
 ***********************************************************************/
int SimpleLR:: GOTO(std::unordered_set<std::string>&from_itemSet, std::unordered_set<std::string>&to_itemSet, std::string Symbol) 
{
    for (std::unordered_set<std::string>::iterator it = from_itemSet.begin(); 
    it!=from_itemSet.end();it++) 
    {
            ...
            if (1 == wantflag) 
            {
                std::string newrule = rules_vec[0]+":";
...
int SimpleLR::GenerateItemSets()
{
    ...
    //initial itemset
    std::string initialstr="Start:dott,"+StartSymbol;
    itemSet.insert(initialstr);
    // construct closure
    Closure(itemSet);
    m_closures.push_back(itemSet);
    std::queue<std::unordered_set<std::string> > ques;
    ques.push(itemSet);
    while (! ques.empty()) 
    {
        itemSet = ques.front();
        ques.pop();
        for (std::unordered_set<std::string>::iterator it = m_symbols.begin(); it != m_symbols.end();it++)
        {
            std::unordered_set<std::string> to_itemSet;
            GOTO(itemSet, to_itemSet,*it);
            if (to_itemSet.size() > 0) 
            {
                bool flag = false;
                Closure(to_itemSet);
                for (std::vector<std::unordered_set<std::string> >::iterator closure_it = m_closures.begin(); closure_it != m_closures.end(); closure_it++) 
                {
                    if (to_itemSet.size() == (*closure_it).size()) 
                    {
                        bool same = true;
                        for (std::unordered_set<std::string>:: iterator subit = to_itemSet.begin(); subit != to_itemSet.end(); subit++ ) 
                        {
...
int SimpleLR::NextEdge() 
{
  for (int i = 0;i < m_states_count; i++) 
  {
      ...
      std::vector<std::string> gotolist;
      for (std::unordered_set<std::string>::iterator it = itemSet.begin(); it != itemSet.end(); it++)
      {
          std::vector<std::string>rules_vec;
          SplitString((*it),rules_vec,":,");
          if (rules_vec.size() < 2)
      ...
      if (gotolist.size() > 0) {
        m_closureNext.insert(std::make_pair(i,newline));
      }
  }
  return 0;
}
...
int SimpleLR::ConstructTables() 
{
    ...
    std::string acc_rule="Start:"+StartSymbol+","+"dott";
    ...
    for (int i = 0;i < m_states_count;i++) 
    {
        ...
        if (m_closures[i].count(acc_rule)) 
        {
            ...
            else 
            {
                ...
                std::vector<std::string> myvectemp;
                strcpy(temp2,ActionTable[i][m_terms_count]);
                SplitString(temp2,myvectemp,",");
                bool toadd = true;
                for (std::vector<std::string>::iterator itt = myvectemp.begin();itt!=myvectemp.end();itt++)
                {
        ...
        for (std::unordered_set<std::string,int>::iterator it = m_closures[i].begin(); it != m_closures[i].end(); it++ ) 
        {
            std::vector<std::string>rules_vec;
            SplitString(*it,rules_vec,":,");
            if (rules_vec.size() < 2)
            ...
            std::string rule_prototype = "";
            std::string head = rules_vec[0];
            rule_prototype = head+":";
            for (int j = 1; j < int(rules_vec.size());j++)
            {
                ...
                if (isdottend) 
                {
                    ...
                    {
                        std::string followstr = m_followset[head];
                        std::vector<std::string> follow_vec;
                        ...
                        for (std::vector<std::string>::iterator subit=follow_vec.begin(); subit!=follow_vec.end(); subit++) 
                        {
        ...
        std::vector<std::string> nextvariables;
        if (m_closureNext.count(i))
        {
            SplitString(m_closureNext[i],nextvariables,":");
        }
        for (std::vector<std::string>::iterator it = nextvariables.begin(); it != nextvariables.end();it++)
        {
            // contruct action table;
            if (m_terminals.count(*it)) 
            {
                std::unordered_set<std::string> toItemSet;
                ...
                if (toItemSet.size() > 0) 
                {
                    ...
                    {
                        bool inflag = true;
                        for (std::unordered_set<std::string>::iterator subit = toItemSet.begin(); subit != toItemSet.end(); subit++) 
                        {
        ...
        if (m_nonterminals.count(*it)) 
        {
            ...
            if (toItemSet.size() > 0) 
            {
                ...
                {
                    bool inflag = true;
                    for (std::unordered_set<std::string>::iterator subit = toItemSet.begin(); 
